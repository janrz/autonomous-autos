...
to adjust-speed
  ...
  ;; if so, set own speed to front turtle's speed and decelerate. If no front
...
to check-left-moving
  ifelse (([speed] of one-of (turtles-at 1 4)) > 0.01) or (not any? turtles-at 1 4) [ set left-moving? true ] [ set left-moving? false ]
end

;; check if vehicles in lane left of vehicle are moving
to check-right-moving
  ifelse (([speed] of one-of (turtles-at 1 -4)) > 0.01) or (not any? turtles-at 1 -4) [ set right-moving? true ] [ set right-moving? false ]
end
...
to move-all-right
  ask turtles [ move-right ]
end

to move-all-left
  ask turtles [ move-left ]
end
